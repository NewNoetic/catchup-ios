...
/// ShowTaps displays your taps and swipes when you're presenting or demoing.
/// Change the options to customise ShowTaps.
public final class ShowTaps: NSObject {
    
    /// Defines if and when ShowTaps should be enabled.
    ///
    /// - always:    ShowTaps is always enabled.
    /// - never:     ShowTaps is never enabled.
    /// - debugOnly: ShowTaps is enabled while the `DEBUG` flag is set and enabled.
    ...
    /// Whether ShowTaps is enabled.
    /// ShowTaps automatically enables itself by default.
    /// (`.always` by default)
    @objc public static var enabled: ShowTaps.Enabled = .always
    
    /// The fill (background) colour of the visual touches.
    /// If set to `.auto`, ShowTaps automatically uses the stroke color with 50% alpha.
    /// (`.auto` by default)
    ...
    /// (`.standard` by default)
    public static var disappearAnimation: ShowTaps.Animation = .standard
...
public extension UIColor {
    ...
    /// Represents a ShowTaps-defined "automatic" color.
    /// For example, setting `ShowTaps.fillColor` to `.auto` results in a fill color that is 50% alpha of the stroke color.
...
class TouchView: UILabel {
    ...
    convenience init(touch: UITouch, relativeTo view: UIView) {
        ...
        self.init(frame: CGRect(x: location.x - ShowTaps.size.width / 2,
                                y: location.y - ShowTaps.size.height / 2,
                                width: ShowTaps.size.width,
                                height: ShowTaps.size.height))
        style(with: touch)
    }
    ...
    func update(with touch: UITouch, relativeTo view: UIView) {
        let location = touch.location(in: view)
        frame = CGRect(x: location.x - ShowTaps.size.width / 2, y: location.y - ShowTaps.size.height / 2, width: ShowTaps.size.width, height: ShowTaps.size.height)
        if ShowTaps.shouldShowForce {
    ...
    func disappear() {
        UIView.animate(withDuration: 0.2, delay: ShowTaps.disappearDelay, options: [.beginFromCurrentState], animations: {
            switch ShowTaps.disappearAnimation {
    ...
    private func style(with touch: UITouch) {
        layer.cornerRadius = ShowTaps.size.height / 2
        layer.borderColor = ShowTaps.strokeColor.cgColor
        layer.borderWidth = ShowTaps.strokeWidth
        backgroundColor = ShowTaps.fillColor == .auto ? ShowTaps.strokeColor.withAlphaComponent(0.5) : ShowTaps.fillColor
        text = ShowTaps.shouldShowMultipleTapCount && touch.tapCount > 1 ? "\(touch.tapCount)" : nil
        textAlignment = .center
        textColor = ShowTaps.multipleTapCountTextColor
        font = ShowTaps.multipleTapCountTextFont
        clipsToBounds = true
...
extension UIWindow {
    ...
    @objc private func swizzled_sendEvent(_ event: UIEvent) {
        ...
        guard ShowTaps.shouldEnable else { return removeAllTouchViews() }
        event.allTouches?.forEach {
            if ShowTaps.shouldIgnoreApplePencilEvents && $0.isApplePencil { return }
            switch $0.phase {
